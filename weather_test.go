...
func TestGetWeather(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case "London":
			...
			w.Write([]byte(`{
				"main": {"temp": 15.5},
				"weather": [{"description": "cloudy", "id": 802}]
			}`))
		case "InvalidCity":
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			gotTemp, gotStatus, gotIcon, err := getWeather("test-api-key", tt.city, tt.units)
			...
			if gotIcon != tt.wantIcon {
				t.Errorf("getWeather() gotIcon = %v, want %v", gotIcon, tt.wantIcon)
			}
		})
	}
}
...
func TestGetWeatherNetworkError(t *testing.T) {
	...
	_, _, _, err := getWeather("test-api-key", "London", "metric")
	if err == nil {
